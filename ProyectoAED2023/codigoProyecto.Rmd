...
colnames(tabla_zonas)[24] <- "V_máx viento"
...
dias_semana <- unique(`Avda. Francia`$`Dia de la semana`)
par(mfrow = c(5, 2))
...
for (dia in orden_dias) {
  ...
  if (nrow(df_media_dia) > 0) {
    ...
    p <- ggplot(df_media_dia, aes(x = Hora)) +
      ...
      xlab("Hora") +
      ylab("Concentración") +
      scale_color_manual(values = c("NO" = "red", "NO2" = "blue", "NOx" = "green", "O3" = "purple", "SO2" = "orange", "CO" = "brown")) +
      labs(color = "Código") +
...
`Avda. Francia`$gases_total <- rowSums(`Avda. Francia`[, c('NO', 'NO2', 'NOx', 'O3', 'SO2', 'CO')], na.rm = TRUE)
